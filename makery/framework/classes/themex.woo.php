...
class ThemexWoo {
	...
	public static function init() {
		...
		if(self::isActive()) {
			...
			add_action('woocommerce_single_product_summary', 'print_sizeguide_link', 20);
		...
		if(self::isActive()) {
			if($ID==1) {
				...
				if(!empty($object)) {
					...
					if($object->post->post_title==__('Untitled', 'makery')) {
						$product['title']='';
					}
					...
					if(!empty($object->product_image_gallery)) {
                        $product['images']=$object->product_image_gallery;//explode(',', $object->product_image_gallery); // to fix issue : "explode param2 should be string, array given"
					}
	...
	public static function updateOrder($ID, $old_status, $new_status) {
		...
		if(!empty($post)) {
			...
			$order=wc_get_order($ID);
			ThemexShop::updateBalance($order->post->post_author, array(
				'order' => $ID,
			));
			...
			if($new_status=='processing') {
				...
				if(!empty($content)) {
					...
					$keywords=array(
						'username' => $user->user_login,
						'order' => '<a href="'.ThemexCore::getURL('shop-order', $order->id).'">#'.$order->get_order_number().'</a>',
					);
