...
jQuery(document).on("click", "#vertical-menu", function () {
	alert("description menu clicked");
}});


jQuery(document).on("click", "#vertical-menu h3", function () {
	alert("description menu clicked");
...
jQuery(document).on("click", "#vertical-menu-2 h3", function () {
    //slide up all the link lists
    $("#vertical-menu-2 ul ul").slideUp();
    $(".plus",this).html("+");
    //slide down the link list below the h3 clicked - only if its closed
    if (!$(this).next().is(":visible")) {
        $(this).next().slideDown();
